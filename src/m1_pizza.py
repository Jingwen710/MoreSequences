...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(125.0, 50.0),  # All numbers are approximate.
                rg.Point(100.0, 125.0),
                rg.Point(75.0, 100.0),
                rg.Point(100.0, 75.0)]
    circle = rg.Circle(rg.Point(100, 100), 25)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 1:  80 purple dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    centers = generate_points_on_circle(circle, number_of_points)
    for k in range(len(centers)):
        circle_new = rg.Circle(centers[k], 10)
        circle_new.fill_color = color
        circle_new.attach_to(window)
        centers[k].attach_to(window)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dots = generate_points_on_circle(circle, number_of_slices)
    center = circle.center
    for k in range(len(dots)):
        start = dots[k]
        end = center
        slice = rg.Line(start, end)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    # Tests 3 (on another window):
    title = 'POLYGON tests 3. 4 segments (black lines on a lime green circle'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dots = generate_points_on_circle(circle, number_of_segments)
    ...
    line1 = rg.Line(dots[len(dots) - 1], dots[0])
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  300 segments with hops_to_next_point = 61 (black lines, thickness=1) on a yellow-filled circle.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dots = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(dots)):
        start = dots[k]
        end = dots[(k + hops_to_next_point) % number_of_lines]
        line = rg.Line(start, end)
        line.attach_to(window)
        line.arrow = 'last'
        line.color = color
